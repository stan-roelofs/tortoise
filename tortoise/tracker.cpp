...
namespace tortoise
{
    ...
    std::map<std::string, std::string> TrackerRequest::GetParameters() const
    {
        std::map<std::string, std::string> params;
        params["info_hash"] = std::string((const char *)info_hash.GetBytes().data(), info_hash.GetBytes().size());
        params["peer_id"] = peer_id.Get();
        params["port"] = std::to_string(port);
        params["uploaded"] = std::to_string(uploaded);
        params["downloaded"] = std::to_string(downloaded);
        params["left"] = std::to_string(left);
        if (compact.has_value())
            params["compact"] = compact.value() ? "1" : "0";
        if (no_peer_id.has_value())
            params["no_peer_id"] = no_peer_id.value() ? "1" : "0";
        if (event.has_value())
        {
            switch (event.value())
            {
    ...
    TrackerResponse::TrackerResponse(const std::string &) : interval(0), min_interval(0), complete(0), incomplete(0)
    {
        // TODO
    }

    bool TrackerResponse::Success() const
    {
        return failure_reason.has_value();
    }

    bool TrackerResponse::Failure() const
    {
        return !Success();
    }
